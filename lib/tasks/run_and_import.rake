task :import_report, [:filename] => :environment do |t, args|
  doc = File.open("spec/reports/SPEC-uptime-verification.xml") { |f| Nokogiri::XML(f)}
...
task :run_and_import => [:uptime, :import_report]

def set_errors(doc_testcase)
